...
This guide explains how to configure Zing for multi-user mode with PostgreSQL database.

## Architecture Overview

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  API clients    │────▶│  Backend API    │────▶│   PostgreSQL    │
│  (any)          │     │   (Flask)       │     │   Database      │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                               │
                        ┌──────┴──────┐
...
```bash
# Run init.sql
psql -U zing -d zing -f server/migrations/init.sql
```
...
export DATABASE_URL=postgresql://zing:your_password@localhost:5432/zing
...
```bash
# Check SQLite path
ls -la server/data/zing.db

# Check PostgreSQL tables
psql -U zing -d zing -c "\dt"
```
