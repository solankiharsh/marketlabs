...
ALERT_MESSAGES = {
    'zh-CN': {
        'price_above': '🔔 Price alert: {symbol} ${current_price:.4f} has exceeded ${threshold:.4f}',
        'price_below': '🔔 Price alert: {symbol} ${current_price:.4f} has dropped below ${threshold:.4f}',
...
def _build_html_report(
    ...
    # Text translations (English only; locale can be extended via i18n)
    texts = {
        'title': 'Portfolio AI Analysis Report',
        'subtitle': 'Generated by Zing Fast AI Analysis Engine',
...
def _build_telegram_report(
    ...
    if is_zh:
        ...
        for pa in position_analyses:
            ...
            text = {'BUY': 'Buy', 'SELL': 'Sell', 'HOLD': 'Hold'}.get(decision, 'Hold')
            pnl = pa.get('pnl', 0)
            pnl_pct = pa.get('pnl_percent', 0)
            pnl_s = '+' if pnl >= 0 else ''
            ...
            lines.append(f"   🎯 Recommendation: <b>{text}</b> (confidence {pa.get('confidence', 50)}%)")
        ...
        lines.extend([
            ...
            "<i>Generated by Zing multi-agent system</i>"
        ])
    else:
        lines = [
            "<b>📊 Portfolio AI Analysis Report</b>",
...
def notify_strategy_signal_for_positions(market: str, symbol: str, signal_type: str, signal_detail: str, user_id: int = None):
    ...
    try:
        ...
        for pos in positions:
            ...
            message = f"""Strategy {signal_type} signal!
...
Please check if your position needs adjustment."""
