...
class SignalNotifier:
    ...
    Notification config:
    - Users configure their own notification settings in profile (telegram_bot_token, telegram_chat_id, email, etc.)
    - When creating strategies/monitors, the system uses the user's configured notification targets.

    Shared service config (set by admin in system settings):
    - SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, SMTP_FROM, SMTP_USE_TLS (email, shared by all users)
    - TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER (SMS, shared by all users)
    ...
    def __init__(self) -> None:
        ...
        # Shared SMTP config (admin configures in system settings)
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        - Signing verification: notification_config.targets.webhook_signing_secret
        - Auto retry once on 429/5xx
        """
        if not url:
        ...
        headers: Dict[str, str] = {
            ...
            "User-Agent": "Zing/1.0 (+https://www.zing.com)",
        }
    ...
    def _notify_telegram(
        ...
        *,
        ...
        # User must configure telegram_bot_token in profile
